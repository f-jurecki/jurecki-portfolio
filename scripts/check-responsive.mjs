...
const widths = [320, 360, 390, 430, 768, 978, 1024, 1025, 1280, 1440, 1501, 1536];
			...
			try {
				...
				const audit = await page.evaluate(() => {
					...
					const raster = images.filter((image) => {
						...
						return src && image.getAttribute('aria-hidden') !== 'true' && !src.startsWith('data:image/svg+xml');
					});
					return {
						...
						headerGap: (() => {
							...
							if (!brand || !nav || !actions || getComputedStyle(nav).display === 'none') return null;
							const brandRect = brand.getBoundingClientRect();
							const navRect = nav.getBoundingClientRect();
							const actionsRect = actions.getBoundingClientRect();
							return Math.min(actionsRect.left - brandRect.right, navRect.left - actionsRect.right);
						})(),
